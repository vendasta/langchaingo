...
import (
	...
	"github.com/tmc/langchaingo/llms/googleaiv2/vertex"
...
func newGoogleAIClient(t *testing.T, opts ...googleaiv2.Option) *googleaiv2.GoogleAI {
	t.Helper()
	...
	opts = append(opts,
		googleaiv2.WithRest(),
		googleaiv2.WithAPIKey("test-api-key"),
		googleaiv2.WithHTTPClient(rr.Client()),
	)

	llm, err := googleaiv2.New(context.Background(), opts...)
	require.NoError(t, err)
	return llm
}

func newVertexClient(t *testing.T, opts ...googleaiv2.Option) *vertex.Vertex {
	t.Helper()
...
var testConfigs = []testConfig{
	...
	{testToolsWithInterfaceRequired, nil},
	{
		...
		[]googleaiv2.Option{googleaiv2.WithHarmThreshold(googleaiv2.HarmBlockMediumAndAbove)},
	},
...
func getHTTPTestClientOptions() []googleaiv2.Option {
	client := &http.Client{Transport: &testRequestInterceptor{}}
	return []googleaiv2.Option{googleaiv2.WithRest(), googleaiv2.WithHTTPClient(client)}
}

// getCustomEmbeddingModelTestOptionsWithGRPC creates options to connect to a fake gRPC server.
func getCustomEmbeddingModelTestOptionsWithGRPC(t *testing.T, model string) []googleaiv2.Option {
	t.Helper()
	...
	return []googleaiv2.Option{
		googleaiv2.WithDefaultEmbeddingModel(model),
		googleaiv2.WithGRPCConn(conn),
	}
}
